...
func FetchPuzzle() (types.Puzzle, error) {
	row := db.QueryRow(`
    SELECT p.id, p.row_size, p.col_size, p.clues,
        COALESCE(pt.title, 'Untitled') AS title,
        COALESCE(a.name, 'Anonymous') AS author
    FROM puzzles p
    LEFT JOIN puzzle_titles pt ON p.id = pt.puzzle_id AND pt.language = 'ko'
    LEFT JOIN puzzle_metadata pm ON p.id = pm.puzzle_id 
    LEFT JOIN authors a ON pm.author_id = a.id
    ORDER BY RANDOM()
    LIMIT 1`)
